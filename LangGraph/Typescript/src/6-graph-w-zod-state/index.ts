...
const TimedMessage = z.object({
  message: z.string(),
  timestamp: z.number(),
});

const SimpleGraphState = z.object({
  step: z.number(),
  messages: z.array(TimedMessage),
  initialTime: z.number().optional(), // Optional property instead of Partial Sate in the nodes
  totalTimeElapsed: z.number().optional(),
});

// First node
function firstNode(
  state: z.infer<typeof SimpleGraphState>
): z.infer<typeof SimpleGraphState> {
...
const graphBuilder = new StateGraph(SimpleGraphState)
  // Add our nodes
